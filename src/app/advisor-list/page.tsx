...
function AdvisorListContent() {
    ...
    fetch('https://www.lifepage.in/n/api/SearchByName', {
      ...
      .then((data) => {
        ...
        mainList.forEach((item) =>
          displayedSessionIds.current.add(String(item.sessionid))
        );
    ...
    fetch('https://www.lifepage.in/n/api/SimilarTalk', {
      ...
      .then((data) => {
        ...
        const filtered: Advisor[] = data.data.filter(
          (item: Advisor) =>
            !displayedSessionIds.current.has(String(item.sessionid))
        );
  ...
  const aiRedirect = async () => {
    ...
    sessionStorage.setItem('fieldOfStudy', careerRaw);

    router.push(`/articles?fieldOfStudy=${encodeURIComponent(careerRaw)}`);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-white text-[#262626] flex flex-col font-sans">
      {/* 🔹 Top Navbar */}
      <Navbar />

      {/* Main Content Area */}
      <main className="flex-1 w-full flex flex-col justify-center items-center p-3 sm:p-5 box-border">
        {/* Container Box */}
        <div className="w-full max-w-[1170px] bg-[#ffc000] border-2 border-[#505050] p-5 sm:p-10 rounded-lg text-center box-border shadow-md">
          <h2 className="text-xl sm:text-2xl font-bold mb-5 text-gray-900 leading-tight">
            Choose an Advisor for{' '}
            {careerRaw || (careerRaw ? uppercaseFirst(careerRaw) : 'Your Path')}
          </h2>
          ...
          >
            <div className="flex flex-col items-center">
              ...
              />
              <b className="text-lg text-[#ffc000]">{careerRaw || 'AI Guide'}</b>
              <b className="text-sm font-semibold tracking-wide">
                AI Career Advisor
              </b>
          ...
          {isLoading && (
            <p className="text-gray-800 font-semibold my-5">
              Loading available mentors...
            </p>
          )}
          {errorMsg && (
            <p className="text-red-700 font-medium my-5">{errorMsg}</p>
          )}
                ...
                >
                  <div className="flex flex-col items-center">
                    <img
                      src={
                        item.profilepic
                          ? `https://storage.googleapis.com/lifepage-plan/amscareerprofile/${item.profilepic}`
                          : '/contact.png'
                      }
                      className="w-[100px] h-[100px] rounded-full object-cover border-2 border-gray-500 mb-3"
                      alt={item.name}
                    />
                    <b className="text-md text-[#ffc000] leading-tight mb-1">
                      {item.topic}
                    </b>
