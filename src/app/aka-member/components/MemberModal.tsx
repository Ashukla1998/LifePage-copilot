...
const CREATE_API_URL = 'https://www.lifepage.in/n/api/AkaMember';
...
export default function MemberModal({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        degree: typeof initialData.degree === 'string' 
          ? initialData.degree.replace(/[{}\[\]\\"]/g, '').trim() 
          : Array.isArray(initialData.degree) 
            ? (initialData.degree as string[]).join(' | ') 
            : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.role.trim() || !formData.category.trim()) {
      setErrorMsg('Name, Role, and Category are required.');
      return;
    }
    ...
    try {
      const isEditing = Boolean(initialData?.memberid);
      const url = isEditing ? EDIT_API_URL : CREATE_API_URL;
      ...
      const payload = {
        ...
        degree: formData.degree ? formData.degree.trim() : null,
        category: formData.category.trim(),
        experience: formData.experience ? formData.experience.trim() : null,
        description: formData.description ? formData.description.trim() : null,
        is_active: initialData ? initialData.is_active : true,
        ...(isEditing ? { memberid: initialData?.memberid } : {}),
      };
